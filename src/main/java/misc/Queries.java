...
//        return "select BILLNO, BILLDATE, SITE_CODE, NETSALES, LPCARDNO, NETSALES from mmpl.V_EKB_POSBILL as bill FULL OUTER JOIN mmpl.V_EKB_CUST_SALE as sale ON bill.BILLNO=sale.BILLNO where billdate >='"+start_date+"' and billdate < '"+end_date+"'" ;
        return "with temp as (select billno, admsite_code, listagg(cat1 || ',' || icode || ',' || netamt || ',' || saleqty || ',' || (saleqty*netamt), ';') within group (order by billno) \"ITEMS\" from mmpl.V_EKB_CUST_SALE where billdate>='01-Jan-18' and billdate<'02-Jan-18' and billno='CM/00027132/18' group by billno, admsite_code) select c.billno, c.admsite_code, c.items, b.netsales, b.lpcardno, b.billdate from mmpl.V_EKB_POSBILL b join temp c on c.billno=b.billno and c.admsite_code=b.site_code";
    }
